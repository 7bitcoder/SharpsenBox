...
namespace bc {
	gm::IGameManager* bc::Get< gm::IGameManager>::component_ = nullptr;
}
namespace gm {
	...
	GameManager::GameManager() : uninstaller_(new GameUninstaller), bc::Get<IGameManager>(this) {};


	Q_INVOKABLE void GameManager::installGame(int id, QString path, bool shortcut) {
		if (!lock_) {
			lock();
			auto& game = bc::Get <cf::IConfig>::get().getGame(id);
			...
			bc::Get <im::IInstalationManager>::get().updateGame(game);
		}
	}
	...
	Q_INVOKABLE void GameManager::unistallRequest(int id) {
		Gameid_ = id;
		auto& dialog = bc::Get < dl::IDialog > ::get();
		dialog.setInfo(QString("Are you sure you want to remove ") + bc::Get <cf::IConfig>::get().getGameName(id));
		dialog.show();
	}
	...
	void GameManager::uninstall(bool dialogValue) {
		if (dialogValue) {
			lock();
			auto& lb = bc::Get < lb::ILoadingBar > ::get();
			lb.reset();
	...
	void GameManager::uninstallation(int id) {
		bc::Get <cf::IConfig>::get().getGame(id).installed = false;
		unLock();
		auto& lb = bc::Get < lb::ILoadingBar > ::get();
		lb.reset();
